...
namespace SecureChat.TripleDES
{
    public class DES
    {
        ...
        private static readonly byte[] PermutedChoiceFirstTable = {
            57, 49, 41, 33, 25, 17, 9,
            1,  58, 50, 42, 34, 26, 18,
            10, 2,  59, 51, 43, 35, 27,
            19, 11, 3,  60, 52, 44, 36,
        ...
        private static readonly byte[] PermutedChoiceSecondTable = {
            14, 17, 11, 24, 1,  5,
            3,  28, 15, 6,  21, 10,
            23, 19, 12, 4,  26, 8,
            16, 7,  27, 20, 13, 2,
        ...
        private static readonly byte[] InversePermutationTable = {
        ...
        private static int[] Permute(int[] inputBits, int[] keyBits, bool isDecrypt)
        {
            ...
            {
                newBits[index] = inputBits[InitialPermutation[index] - 1];
            }
            ...
            for (i = 0; i < 28; i++)
            {
                C[i] = keyBits[PermutedChoiceFirstTable[i] - 1];
            }
            for (; i < 56; i++)
            {
                D[i - 28] = keyBits[PermutedChoiceFirstTable[i] - 1];
            }
            ...
            for (i = 0; i < 64; i++)
            {
                Output[i] = output[InversePermutationTable[i] - 1];
            }
        ...
        private static int[] KS(int round, int[] key)
        {
            ...
            {
                Kn[i] = CnDn[PermutedChoiceSecondTable[i] - 1];
            }
        ...
        private static int[] SBlock(int[] bits)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                int x = SBoxes[i][(iRow * 16) + iColumn];
            ...
            for (int i = 0; i < 32; i++)
            {
                Output[i] = output[PermutationTable[i] - 1];
            }
            return Output;
        }
